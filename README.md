...
`mojV Auth Helper` is a local Chromium/Xvfb service. It is **not** the main Home Assistant integration and it is not the preferred transport.

The main integration lives at:
...
1. Python compilation and contract/regression tests.
2. Native `amd64` container build.
3. Runtime `/health` assertion.
4. Xvfb process check.
5. Chromium and ChromeDriver execution checks.
6. `aarch64` image build through QEMU/Buildx.
